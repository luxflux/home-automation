  ...
  class Show
    ...
    def call(params)
      ...
      @values.each do |measurement|
        query = "SELECT mean(value) FROM #{measurement.kind} WHERE time > now() - 26h GROUP BY time(5m)"
      ...
      @states.each do |measurement|
        query = "SELECT mean(state) FROM #{measurement.kind} WHERE time > now() - 26h GROUP BY time(5m)"
        data = influxdb.query query
        data.map! { |result| result['values'] }
        data.flatten!
        @measurements << { name: measurement.kind, current: measurement.current, data: data }
